...
class OrderDetailsResource extends JsonResource
{
    ...
    {
        $vatTax = Tax::where('status', 5) // Assuming Status::ACTIVE is 1
            ->where(function ($query) {
                $query->where('name', 'VAT')
                    ->orWhere('code', 'VAT14');
            })
            ->first();

        $vatTaxRate = $vatTax ? $vatTax->tax_rate : null;
        $priceAfterDiscount = $this->subtotal - $this->discount; 
        $priceAfterTax = $priceAfterDiscount + ($priceAfterDiscount * ($vatTaxRate / 100));
        $Taxvalue = $priceAfterTax - $priceAfterDiscount;
        return [
            'id'                                  => $this->id,
            ...
            'VAT_tax_rate'                        => AppLibrary::currencyAmountFormat($vatTaxRate),
            'tax_value'                           => AppLibrary::currencyAmountFormat($Taxvalue),
            'price_after_tax'                     => AppLibrary::currencyAmountFormat($priceAfterTax),
        ];
    }
}
