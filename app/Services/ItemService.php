...
{
    try {
        $query = Item::where('is_featured', 5) // Assuming Ask::YES is 5 based on previous SQL
                    ->where('status', 5); // Assuming Status::ACTIVE is 5 based on previous SQL

        // Apply brand filter only if a valid brandId is provided
        if ($brandId) {
            $query->where('brand_id', $brandId);
        }

        // Remove branch-specific filtering to get items across all branches
        // No whereHas('branches') clause is needed
