...
class FrontendOrderService
{
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                if ($request->order_type == OrderType::DELIVERY) {
                    ...
                    if (!$address) {
                        throw new Exception('Address not found.', 422);
                    }
                    ...
                    foreach ($areas as $area) {
                        ...
                        if ($this->isPointInPolygon($userLat, $userLon, $coordinates)) {
                            $isInZone = true;
                            $deliveryFee = $area->delivery_fees; // Set delivery fee from the area
                            break;
                        }

                        // Calculate distance to nearest point in the polygon
                        $distance = $this->distanceToNearestPoint($userLat, $userLon, $coordinates);
                        if ($distance < $minDistance) {
                            $minDistance = $distance;
                            $nearestZoneName = $area->name;
                        }
                    }

                    if (!$isInZone) {
                        $distanceInKm = $minDistance / 1000; // Convert meters to kilometers
                        throw new Exception("The delivery address is outside all active zones. Nearest zone ('$nearestZoneName') is approximately $distanceInKm km away.", 422);
                    }
                }
                ...
                $this->frontendOrder = FrontendOrder::create(
                    $request->validated() + [
                        ...
                        'delivery_fee'     => $request->order_type == OrderType::DELIVERY ? $deliveryFee : 0, // Add delivery fee if DELIVERY
                    ]
                );
    ...
    private function distanceToNearestPoint($lat, $lon, $polygon): float
    {
        $minDistance = PHP_FLOAT_MAX;
        foreach ($polygon as $point) {
            $pointLat = $point['lat'];
            $pointLon = $point['lng'];
            $distance = $this->haversineDistance($lat, $lon, $pointLat, $pointLon);
            $minDistance = min($minDistance, $distance);
        }
        return $minDistance;
    }
    private function isPointInPolygon($lat, $lon, $polygon): bool
    {
        $inside = false;
